...
const en: CommunityUiCopy = {
  ...
  attachments: { title: "Attachments", chooseFile: "Choose file", dropHint: "Drop images, videos, audio or documents here.", uploadFailed: "Upload failed.", maximumFiles: (count) => `You can add up to ${count} files here.`, invalidFile: "The file type or size is invalid.", removeFailed: "The file could not be removed.", preparing: "Preparing", uploading: (progress) => `Upload ${progress}%`, securityCheck: "Security check", ready: (size) => `Ready | ${size}`, remove: (name) => `Remove ${name}`, removeAttachment: "Remove attachment", open: (name) => `Open ${name}`, video: (name) => `Video ${name}`, audio: (name) => `Audio ${name}` },
