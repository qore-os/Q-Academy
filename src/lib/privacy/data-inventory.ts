...
export const PRIVACY_DATA_INVENTORY = {
  ...
  video_description_jobs: table(
    "video_description_jobs",
    relation(
      "mixed",
      [
        "requested_by_id",
        "requester_subject_reference",
        "source_asset_reference_id",
        "block_reference_id",
      ],
      ["users", "media_assets", "content_blocks"],
      "The requester is personal attribution while the job otherwise follows shared course content.",
    ),
    exportPolicy(
      "metadata_only",
      "Include only status and lifecycle metadata; never export internal idempotency, digest, claim or scheduling state.",
      {
        excludedColumns: [
          "request_key",
          "source_content_sha256",
    ...
    erasurePolicy(
      "unlink",
      ["retention_decision", "shared_resource_review"],
      "Unlink requester attribution while the shared course-content job follows bounded operational retention.",
    ),
    legalHold(["learning", "audit"], "The lifecycle may evidence automated shared-content authoring."),
  ),
