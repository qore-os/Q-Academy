...
type SnapshotSection = SnapshotModule["sections"][number];
...
function modules(snapshot: CourseVersionSnapshot | null) {
  return new Map<string, LocatedModule>(
    (snapshot?.modules ?? []).map((value, index) => [value.id, { value, index }]),
  );
}

function sections(snapshot: CourseVersionSnapshot | null) {
  const result = new Map<string, LocatedSection>();
  for (const [moduleIndex, learningModule] of (snapshot?.modules ?? []).entries()) {
    for (const [index, section] of learningModule.sections.entries()) {
...
function legacyVisibility(value: { status: string; visibility?: string }) {
  return value.visibility ?? (value.status === "published" ? "visible" : "draft");
}

function moduleAccess(value: SnapshotModule) {
  return {
    accessMode: value.accessMode ?? (value.dripDays > 0 ? "delay_days" : "visible"),
...
export function diffCourseSnapshots(
  ...
  for (const goal of published?.learningGoals ?? []) {
    if (!newGoals.has(goal.id)) {
      add("goals", "removed", trimmed(goal.text, copy.fallback.goal), copy.details.goalRemoved);
    }
  }
  for (const [index, goal] of (draft.learningGoals ?? []).entries()) {
    ...
    if (!previous) add("goals", "added", label, copy.details.goalAdded);
      ...
      if (previous.text !== goal.text) add("goals", "updated", label, copy.details.textChanged);
      const previousIndex = (published?.learningGoals ?? []).findIndex((item) => item.id === goal.id);
      if (previousIndex !== index) add("goals", "moved", label, copy.details.orderChanged);
    }
  }

  const oldAuthors = indexById(published?.authors ?? []);
  const newAuthors = indexById(draft.authors ?? []);
  const authorLabel = (author: (NonNullable<CourseVersionSnapshot["authors"]>)[number]) =>
    trimmed(`${author.author.firstName} ${author.author.lastName}`, copy.fallback.author);
  for (const author of published?.authors ?? []) {
    if (!newAuthors.has(author.id)) add("authors", "removed", authorLabel(author), copy.details.authorRemoved);
  }
  for (const [index, author] of (draft.authors ?? []).entries()) {
    ...
    if (!previous) add("authors", "added", label, copy.details.authorAdded);
      ...
      if (!sameValue(previous.author, author.author) || previous.userId !== author.userId) {
        add("authors", "updated", label, copy.details.authorProfileChanged);
      }
      const previousIndex = (published?.authors ?? []).findIndex((item) => item.id === author.id);
      if (previousIndex !== index) add("authors", "moved", label, copy.details.orderChanged);
    }
  }

  const oldWidgets = indexById(published?.widgets ?? []);
  const newWidgets = indexById(draft.widgets ?? []);
  const widgetLabel = (widget: (NonNullable<CourseVersionSnapshot["widgets"]>)[number]) =>
    trimmed(widget.title, widget.type === "author" ? copy.fallback.authorWidget : copy.fallback.widget);
  for (const widget of published?.widgets ?? []) {
    if (!newWidgets.has(widget.id)) add("widgets", "removed", widgetLabel(widget), copy.details.widgetRemoved);
  }
  for (const [index, widget] of (draft.widgets ?? []).entries()) {
    ...
    if (!previous) add("widgets", "added", label, copy.details.widgetAdded);
      ...
      const previousIndex = (published?.widgets ?? []).findIndex((item) => item.id === widget.id);
      if (previousIndex !== index) add("widgets", "moved", label, copy.details.orderChanged);
    }
  }
  ...
  for (const oldModule of oldModules.values()) {
    if (!newModules.has(oldModule.value.id)) {
      add("modules", "removed", trimmed(oldModule.value.title, copy.fallback.module), copy.details.moduleRemoved);
    }
  }
  for (const current of newModules.values()) {
    ...
    if (!previous) add("modules", "added", label, copy.details.moduleAdded);
      ...
      if (orderChanged || indentChanged) {
        const detail = orderChanged && indentChanged
      ...
      if (!sameValue(moduleAccess(previous.value), moduleAccess(current.value))) {
        add("access", "updated", label, copy.details.moduleAccessChanged);
      }
    }
  }

  const oldSections = sections(published);
  const newSections = sections(draft);
  for (const oldSection of oldSections.values()) {
    if (!newSections.has(oldSection.value.id)) {
      add("sections", "removed", trimmed(oldSection.value.title, copy.fallback.section), copy.from(oldSection.module.title));
    }
  }
  for (const current of newSections.values()) {
    const previous = oldSections.get(current.value.id);
    const label = trimmed(current.value.title, copy.fallback.section);
    if (!previous) add("sections", "added", label, copy.in(current.module.title));
    else {
      ...
      if (!sameValue(previousComparable, currentComparable)) {
        add("sections", "updated", label, copy.details.sectionSettingsChanged);
      }
      if (previous.module.id !== current.module.id || previous.index !== current.index) {
        add("sections", "moved", label, copy.in(current.module.title));
      }
    }
  }
  ...
  for (const oldLesson of oldLessons.values()) {
    if (!newLessons.has(oldLesson.value.id)) {
      add("lessons", "removed", trimmed(oldLesson.value.title, copy.fallback.lesson), copy.from(oldLesson.module.title));
    }
  }
  for (const current of newLessons.values()) {
    ...
    if (!previous) add("lessons", "added", label, copy.in(current.module.title));
    else {
      const previousComparable = {
        title: previous.value.title,
      ...
      if (
        ...
        add("lessons", "moved", label, current.section ? copy.in(current.section.title) : copy.in(current.module.title));
      }
    }
  }
  ...
  for (const oldPage of oldPages.values()) {
    if (!newPages.has(oldPage.value.id)) {
      add("pages", "removed", trimmed(oldPage.value.title, copy.fallback.page), copy.from(oldPage.lesson.title));
    }
  }
      ...
      if (
        !sameValue(
          ...
          },
          {
            ...
            status: current.value.status,
            titleSyncedWithLesson:
              current.value.titleSyncedWithLesson ?? false,
          },
        )
      ) {
        add("pages", "updated", label, copy.details.pageSettingsChanged);
      }
      if (previous.lesson.id !== current.lesson.id || previous.index !== current.index) {
  ...
  for (const oldBlock of oldBlocks.values()) {
    if (!newBlocks.has(oldBlock.value.id)) {
      add("blocks", "removed", blockLabel(oldBlock.value), blockDetail(oldBlock));
    }
  }
  for (const current of newBlocks.values()) {
    ...
    if (!previous) add("blocks", "added", label, blockDetail(current));
      ...
      if (!sameSelected(previous.value, current.value, ["type", "title", "required", "data"])) {
        add("blocks", "updated", label, copy.blockContentChanged(blockDetail(current)));
      }
      if (
