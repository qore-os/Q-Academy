...
export async function GET(
  ...
  return handleApi(
    request,
    { scopes: ["modules:read"], action: "module.read", resourceType: "module" },
    async (context) => {
      const learningModule = await moduleForOrganization(
        id,
      ...
      return {
        data: { ...learningModule, lessons: structure },
        resourceId: id,
      };
    },
  );
  ...
  return handleTransactionalApiCommand(
    ...
    },
    {
      ...
      execute: async ({ context, tx, activity }) => {
        ...
        const current = await lockModuleStructure(tx, {
          organizationId: context.organizationId,
          moduleId: id,
        });
